...
if [ "$MINECRAFT_VERSION" == "latest" ]; then
  ...
  echo "User specified \$MINECRAFT_VERSION=\"latest\". Substituting for \"$MINECRAFT_VERSION\"."
elif [ "$MINECRAFT_VERSION" == "latest_snapshot" ]; then
  MINECRAFT_VERSION=$(curl -s "$MANIFEST_URL" | jq -r ".latest.snapshot")
  echo "User specified \$MINECRAFT_VERSION=\"latest_snapshot\". Substituting for \"$MINECRAFT_VERSION\"."
fi
