...
def get_minecraft_download_url(version):
    ...
    if len(desired_versions) == 0:
        raise RuntimeError(
            "Couldn't find Minecraft Version {version} in manifest file {MANIFEST_URL}.".format(
                version, MANIFEST_URL
            )
        )
    elif len(desired_versions) > 1:
        raise RuntimeError(
            "Found more than one record published for version {version} in manifest file {MANIFEST_URL}.".format(
                version, MANIFEST_URL
            )
        )
