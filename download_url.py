...
def get_minecraft_download_url(version):
    ...
    if len(desired_versions) == 0:
        raise RuntimeError(
            "Couldn't find Minecraft Version {} in manifest file {}.".format(version, MANIFEST_URL)
        )
    elif len(desired_versions) > 1:
        raise RuntimeError(
            "Found more than one record published for version {} in manifest file {}.".format(version, MANIFEST_URL)
        )
